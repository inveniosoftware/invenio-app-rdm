...
@pytest.fixture
def set_tmp_config(app):
    config_prev = {}

    def _set_tmp_config(config):
        for key, value in config.items():
            config_prev[key] = app.config[key]
            app.config[key] = value

    yield _set_tmp_config

    for key, value in config_prev.items():
        app.config[key] = value
...
def test_task_file_integrity_report(
    app, record_with_invalid_file_instance, set_tmp_config
):
...
def test_integrity_report_invalid_template(app, invalid_file_instance, set_tmp_config):
    ...
    with mail.record_messages() as outbox:
        ...
        set_tmp_config({"APP_RDM_FILES_INTEGRITY_REPORT_TEMPLATE": None})

        file_integrity_report()
        assert len(outbox) == 0


def test_integrity_report_invalid_addresses(app, invalid_file_instance, set_tmp_config):
...
def test_integrity_report_default_template(
    app, draft_with_invalid_file_instance, set_tmp_config
):
    """Test invalid recipient address."""
